...
with psycopg2.connect(dbname="postgres", user="postgres", password=" ", host="127.0.0.1") as con:
    ...
    if cur.fetchone() is None:
        ...
        level = cur.fetchone()
        level = level[0]
    cur.execute(f"SELECT users_test.user_id, users_test.name, levels_test.level FROM levels_test join users_test on levels_test.level_id = users_test.user_id  WHERE name = '{user_name}'")
    data = cur.fetchall()
    for i in data:
        print(i)
