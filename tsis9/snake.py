...
pause = pygame.image.load(
    r'C:\Users\Admin\Desktop\pp2-22B030386\tsis9\images\Pause-PNG-Image-Transparent.png')
start = pygame.image.load(
    r'C:\Users\Admin\Desktop\pp2-22B030386\tsis9\images\Play-Button-Transparent-Image.png')
...
score = "0"
FPS = 15
score_table = f.render(score, True, 'red')
...
while check:
    ...
    pg.draw.rect(sc, pg.Color("red"), (*apple, 20, 20))
    ...
    if snake_body[-1] == apple:
        ...
        if int(score) % 3 == 0 and int(score) != 0:
            level = str(int(level) + 1)
    for i in snake_body:
        if snake_body.count(i) > 1:
            check = False
    a, b = snake_body[-1]
    if a < -20 or a > 780 or b < 40 or b > 580:
        check = False
    x += speed_x
    y += speed_y
